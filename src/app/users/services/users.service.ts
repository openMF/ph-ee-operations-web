...
export class UsersService {
  ...
  private institueAddress = 'com.myspace.paymenthubee.Institute';
  private govtEntities: any= JSON.parse(sessionStorage.getItem('pheeGovtEntities'));
  private dfsps: any= JSON.parse(sessionStorage.getItem('pheeDfsps'));
  /**
  ...
  getUserTypes(): Observable<any> {
    const userTypes = [
      ...
      { id: 2, name: 'Govt. Entity User', GovtId: this.govtEntities },
      { id: 3, name: 'FSP User', FSPId: this.dfsps },
    ];
              ...
              return response['task-summary']
                .map((task: any, index: number) => {
                  ...
                  if (!userData) {
                    ...
                    fspId: userData.fspId ? (userData.fspId[this.institueAddress] || null) : null,
                    govtId: userData.govtId ? (userData.govtId[this.institueAddress] || null) : null,
  ...
  private mapFormDataToUser(user: any): any {
    ...
    return {
      ...
      attributes: {
        fspId: [user.fspId?.id],
        fspName: [user.fspId?.name],
        govtId: [user.govtId?.id],
        govtName: [user.govtId?.name],
